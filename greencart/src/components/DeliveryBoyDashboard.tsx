...
function DeliveryBoyDashboard({earning}:{earning:number}) {
  ...
  setAssignments((prev) => {
    if (prev.some((a) => a._id === deliveryAssignment._id)) return prev
...
const verifyOtp = async () => {
  ...
  />
          ...
          <div className='mt-6 bg-white rounded-xl border shadow p-6'>
            {!activeOrder.order.deliveryOtpVerification && !showOtpBox && (
             <button
             onClick={sendOtp}
             className='w-full py-4 bg-green-600 text-center text-white rounded-lg flex items-center justify-center'
            ...
            )}
            {
             ...
             <button className='w-full mt-4 bg-blue-600 text-white py-3 text-center rounded-lg flex items-center justify-center' onClick={verifyOtp}>{verifyOtpLoading?<Loader size={16} className='animate-spin text-white text-center'/>:"Verify OTP"}</button>
  ...
  if(!activeOrder && assignments.length==0){
    ...
    return <div className='flex items-center justify-center min-h-screen bg-linear-to-br from-white to-green-50 p-6 mt-10'>
        ...
        <div className='bg-white border rounded-xl shadow-xl p-6'>
          ...
          <CartesianGrid stroke="#ccc" strokeDasharray="5 5" />
           ...
           <YAxis />
            <Tooltip  />
            <Legend />
            <Bar dataKey="earnings" name="Earnings (Rs)" />
